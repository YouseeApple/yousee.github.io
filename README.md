...
本專案採用 [MIT License](./LICENSE) 授權（原作 Copyright (c) 2026 kenyuiyui，授權全文見 [LICENSE](./LICENSE)）。簡單來說：你可以自由使用、複製、修改、散布這份程式碼，包含商業用途，唯一的要求是保留原始的版權聲明與授權條款。程式碼「按現狀提供」，不附帶任何擔保。
